...
public class NetworkTopology {
    ...
    public final static int DEFAULT_HOST_LEVEL = 3;
    ...
    /** min added, return the number of data centers **/
    public int getNumOfDC() {
        netlock.readLock().lock();
        try {
            Node n = chooseRandom(NodeBase.ROOT);
            int i = 3;
            while(n != null && i >0) {
                n = n.getParent();
                --i;
            ...
            if (n != null && i == 0)
                return ((InnerNode)n).getNumOfChildren();
            else
                return 1;
        } finally {
            netlock.readLock().unlock();
    ...
    public boolean isInSameDC( Node node1,  Node node2) {
        if (node1 == null || node2 == null) {
            return false;
        }
        ...
        netlock.readLock().lock();
        try {
            return node1.getParent().getParent()==node2.getParent().getParent();
        } finally {
            netlock.readLock().unlock();
        }
    }
    ...
    public Node chooseRandomFrom(String scope, String excludedScope){
        netlock.readLock().lock();
        try {
            if (excludedScope.startsWith("~")) {
                return chooseRandom(scope, excludedScope.substring(1));
            } else {
                return chooseRandom(scope, null);
            }
        } finally {
