...
<%
    ...
    for (JobHistory.Task task : tasks.values()) {
      ...
      for (TaskAttempt attempt : attempts.values()) {
        long launchTime = attempt.getLong(Keys.START_TIME) ;
        long finishTime = attempt.getLong(Keys.FINISH_TIME) ; 
        if (Values.MAP.name().equals(task.get(Keys.TASK_TYPE))){
          if (mapStarted==0 || mapStarted > launchTime ) {
            mapStarted = launchTime;
          }
          ...
          if (reduceStarted==0||reduceStarted > launchTime) {
            reduceStarted = launchTime ;
          }
          ...
          if (cleanupStarted==0||cleanupStarted > launchTime) {
            cleanupStarted = launchTime ;
          }
          ...
          if (setupStarted==0||setupStarted > launchTime) {
            setupStarted = launchTime ;
          }
