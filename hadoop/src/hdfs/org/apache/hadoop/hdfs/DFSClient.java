...
static int getMaxBlockAcquireFailures(Configuration conf) {
  ...
  private void checkOpen() throws IOException {
    if (!clientRunning) {
    	//LOG.info("==flexfetch== checkopen fail");
  ...
  private static LocatedBlocks callGetBlockLocations(ClientProtocol namenode,
    ...
    try {
//LOG.info("==flexfetch== trying to get block location " + src + "," +
//			start + "," +
//			length);
      return namenode.getBlockLocations(src, start, length);
    } catch(RemoteException re) {
//LOG.info("==flexfetch== callGetBlockLocations exception");
...
//LOG.info("==flexfetch== openinfo 1");
      if (locatedBlocks != null) {
        Iterator<LocatedBlock> oldIter = locatedBlocks.getLocatedBlocks().iterator();
        Iterator<LocatedBlock> newIter = newInfo.getLocatedBlocks().iterator();
        while (oldIter.hasNext() && newIter.hasNext()) {
          if (! oldIter.next().getBlock().equals(newIter.next().getBlock())) {
//LOG.info("==flexfetch== openinfo 2");
     ...
     * # For elf: return null if the new block is prefetched
     */
    private synchronized DatanodeInfo blockSeekTo(long target) throws IOException {
      ...
      while (true) {
        ...
        long bid = targetBlock.getBlock().getBlockId();
        String path = prefetchClient.getPrefetchedBlockPath(bid);
        // block cached, this is elf logic
        if(path != null){
        	blockReader = new LocalBlockReader(path);
            // why return null? read the comments above
        	return null;
        }
    ...
    public synchronized int read(byte buf[], int off, int len) throws IOException {
      ...
      failures = 0;
//LOG.info("==flexfetch== DFSInputStream is reading from pos " + pos);
...
//LOG.info("	==flexfetch== blockreader read from src " + src);            
    ...
    public int read(long position, byte[] buffer, int offset, int length)
      throws IOException {
LOG.info("==flexfetch== DFSInputStream is reading3 file " + src + "," +
				position + "," +
				offset + "," +
				length);
      // sanity checks
  ...
  ****************************************************************/
  ...
  /*
  private CacheBlockManager cacheManager = new CacheBlockManager(0, 1024*1024*64);
  
  class CacheBlockManager{
	  private int blockSlotNum;
	  private CacheBlock[] blkPool = null;
	  private int blockSize = 0;
	  public static final long INVALID_ID = 0;
	  
	  public CacheBlockManager(int slotNum, int size){
		  this.blockSlotNum = slotNum;
		  this.blockSize = size;
		  blkPool = new CacheBlock[slotNum];
		  for(int i = 0; i < slotNum; i++){
			  blkPool[i] = new CacheBlock(INVALID_ID, size);
		  }
	  }

	  synchronized public CacheBlock getCacheBlock(){
		  for(int i = 0; i< blockSlotNum; i++){
			  if(!blkPool[i].isFree()){
				  blkPool[i].activate();
				  return blkPool[i];
			  }
...
*/
  ...
  /* Prefetch blocks given filename, starting offset and length of read
   * */
  public void prefetchBlock(String fileName, long offset, long len) 
  							throws IOException {
	  
      DFSInputStream dfsIn = null;
	  try{
		  dfsIn = open(fileName);
		  if(dfsIn != null){
			  List<LocatedBlock> blocks = dfsIn.getBlockRange(offset, len);
			  
			  if(blocks != null){
				  for(LocatedBlock blk: blocks){
					  // in fact should be only one block in the list
					  long bid= blk.getBlock().getBlockId();
					  
					  //Read data into cache blk and associate with blk id
					  //CacheBlock cacheBlock = cacheManager.getCacheBlock();
					  CacheBlock cacheBlock = new CacheBlock(bid, (int)blk.getBlockSize());
					  cacheBlock.bindInput(dfsIn);
					  cacheBlock.writeWholeBlock(offset, len);
					  String path = cacheBlock.flush();
					  markAsComplete(bid, path);
					  DFSClient.LOG.info("==elf==" + " Block " + bid + "is prefetched");
				  }
			  }
		  }
		  ...
		  System.err.println("NPE during operation: "
					+ npe.toString());
	  } finally {
          if(dfsIn != null){
              dfsIn.close();
  ...
  private PrefetchProtocol prefetchClient = null;
		  
  public void markAsComplete(long bid, String path){
	  //LOG.info("==flexfetch== marking finished block" );
	  prefetchClient.addBlock(bid, path);
  }
  
  // called when init DFSClient
  private void initPrefetchClient() {
	  try{
		  addr = new InetSocketAddress("localhost", 26600);
		  prefetchClient = (PrefetchProtocol) RPC.waitForProxy(PrefetchProtocol.class,
				  PrefetchProtocol.versionID, addr, new Configuration());
		  LOG.info("==elf== prefetch client connected" );
	  } catch (IOException ioe){
		  System.err.println("Fail to connect to prefetch server: " + ioe);
	  }
  }
  
  	//
  	// BlockReader that read from /dev/shm 
  	//
	public static class LocalBlockReader extends BlockReader {
		
		private String inMemFilePath = null;
		private BufferedInputStream ins = null;
		private int currentPosInBlock = 0;
		
		public LocalBlockReader (String strPath) throws IOException {
			super(); // dummy parent constructor
			inMemFilePath = strPath;
			if(!inMemFilePath.startsWith("/dev/shm")){
				throw new IOException("Not reading from memory");
			}
			ins = new BufferedInputStream(new FileInputStream(inMemFilePath));
            LOG.info("==elf== LocalBlockReader created for " + inMemFilePath);
		}
		...
		public synchronized int read(byte[] buf, int off, int len)
			...
			int nread = ins.read(buf, off, len);
			currentPosInBlock += nread;
			return nread;
		}
		...
		public void seek(long pos) throws IOException {
			throw new IOException(
					"Seek() is not supported in BlockInputChecker");
		}
		
		@Override
	    public synchronized long skip(long n) throws IOException {
			long nskip = ins.skip(n);
			currentPosInBlock += n;
			return nskip;
		}
	}
