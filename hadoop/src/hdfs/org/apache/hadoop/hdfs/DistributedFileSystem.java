...
public class DistributedFileSystem extends FileSystem {
  ...
  public FSDataInputStream open(Path f, int bufferSize) throws IOException {
	  f.getName();
//LOG.info("==flexfetch== DistributedFileSystem open is called " + f.toString());
    statistics.incrementReadOps(1);
  ...
  /* Implementation of prefetching operations
   * */
  public void prefetch(Path fileName, long offset, long len) {
	  try{
		  //byte[] buf = new byte[1024*1024*64];
		  dfs.prefetchBlock(getPathName(fileName), offset, len);
	  } catch (IOException e){
		  System.err.println("IOException during prefetching: "
					+ e.toString());
	  }
  }
}
