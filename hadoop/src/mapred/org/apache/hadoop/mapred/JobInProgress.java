...
public class JobInProgress {
    ...
    /**
     * Get prefetch task for this tracker
     **/
    // TODO: check the correctness of this function tmr
    public synchronized PrefetchTask getPrefetchTasks(TaskTracker tracker){
        for(TaskInProgress tip: maps){
            // check if this task need to be prefetched
            // Condition:
            // 1. this task has no local copy
            // 2. this task has not yet been prefetched
            // 3. this task has not been launched normally
            LOG.debug("==elf== checking task " + tip.getTIPId() + " status: " +
                        tip.isDCLocal() + "," + tip.getPrefetchStatus() + "," +
                        tip.isRunnable() + "," + tip.isRunning());
            if(!tip.isDCLocal() && // has no dc-local replica
               tip.getPrefetchStatus() == TaskInProgress.PrefetchStatus.NONE_RUNNING && // not prefetched
               tip.isRunnable() && // not complete in normal execution
               !tip.isRunning()){ // not running in normal execution
                // marking as running will automatically disable normal launching,
                // since we check the prefetch status in findTaskFromList()
                LOG.info("==elf== job " + getJobID() + " is choosing " +
                         tip.getTIPId() + " to prefetch");
                tip.markPrefetchStatus(TaskInProgress.PrefetchStatus.RUNNING);
                return tip.getFuturePrefetch();
            }
        }
        return null;
    }
    ...
    private Map<Node, List<TaskInProgress>> createCache(
        ...
        for (int i = 0; i < splits.length; i++) {
            ...
            //TODO: for elf
            // when create task, check if any tt of the mapreduce cluster
            // has a replica. if not, mark it as 'across datacenter'
            boolean hasReplicaInDC = false;

            for(String host: splitLocations) {
                Node node = jobtracker.resolveAndAddToTopology(host);
                uniqueHosts.add(host);
                LOG.info("tip:" + maps[i].getTIPId() + " has split on node:" + node);
                // Strange, maxLevel does *NOT* mean root has larger level number
                ...
                //TODO: need acquire lock of JT?
                if(jobtracker.hasTrackerWithHostname(host)){
                    hasReplicaInDC = true;
                }
            ...
            // no replica for this split on running task trackers
            if(!hasReplicaInDC){
                LOG.info("==elf== marking task " + maps[i].getTIPId() + " as dc-remote");
                maps[i].markAsRemoteDCTask();
                maps[i].addPrefetchTask(splits[i]);
            }
        }
     ...
     * @param numSlotsPerMap the number of slots required to run a single map task-attempt
     */
     ...
     * This is typically set by schedulers which support high-ram jobs.
     * @param numSlotsPerReduce the number of slots required to run a single reduce
     *              task-attempt
     */
     ...
     * @return the job's configuratioJobConf
     */
    JobConf getJobConf() {
        return conf;
    }
   ...
   */
    ...
    // TODO: does this function change any meta data? if not it's okay for flexfetch, otherwise need to fix
    /**
    ...
    private synchronized TaskInProgress findTaskFromList(
        ...
        while (iter.hasNext()) {
            ...
            // (4) for elf: no prefetching in run
            if (tip.isRunnable() && !tip.isRunning() &&
                    !tip.isPrefetchingRunning()) { // condition (4)
    ...
    // Code block for flexfetch
    //
    private synchronized int findPseudoMapTask(final PseudoTaskTrackerStatus tts,
                                            final int clusterSize,
                                            final int numUniqueHosts,
                                            final int maxCacheLevel,
                                            final double avgProgress) {
        ...
        // 0) Schedule the task with the most failures, unless failure was on this
        //    machine
        tip = findPseudoTaskFromList(failedMaps, tts, numUniqueHosts, false); // TODO: to check this function
        if (tip != null) {
            // Add to the running list
            // code block for flexfetch: skip schedulMap

            //scheduleMap(tip); // <=== comment for flexfetch
            LOG.info("Choosing a failed task as pseudo task" + tip.getTIPId());
            return tip.getIdWithinJob();
        }
        ...
        if (node != null) {
            ...
            for (level = 0;level < maxLevelToSchedule; ++level) {
                List <TaskInProgress> cacheForLevel = nonRunningMapCache.get(key);
                if (cacheForLevel != null) {
                    tip = findPseudoTaskFromList(cacheForLevel, tts,
                            numUniqueHosts,level == 0);
                    if (tip != null) {
                        // Add to running cache
                        //scheduleMap(tip); // <=== do not really schedule the map

                        // ==> not necessary for pseudo schedule
                        /*
                        // remove the cache if its empty
                        if (cacheForLevel.size() == 0) {
                            nonRunningMapCache.remove(key);
     ...
     */
    // <== old comments
    private synchronized TaskInProgress findPseudoTaskFromList(
            Collection<TaskInProgress> tips, TaskTrackerStatus ttStatus,
            int numUniqueHosts,
            boolean removeFailedTip) {

        // NOTE: In pseudo schedule we do not need to remove anything from the given list
    ...
    /////////////////////////////////////////////
    // Code block for datacenter-aware scheduling
    /////////////////////////////////////////////
    public synchronized Task obtainNewMapTaskWithinDatacenter(TaskTrackerStatus tts,
                                                   int clusterSize,
                                                   int numUniqueHosts)
            throws IOException {
        if (!tasksInited) {
        ...
        // Now level 0 - 2 should be considered as 'local'
        int target = findNewMapTask(tts, clusterSize, numUniqueHosts, maxLevel,
                status.mapProgress());
        if (target == -1) {
            return null;
    ...
    public synchronized Task obtainNewMapTaskAcrossDatacenter(TaskTrackerStatus tts,
                                                      int clusterSize,
                                                      int numUniqueHosts)
            throws IOException {
        if (!tasksInited) {
    ...
    public void increaseReplica(TaskInProgress tip, Node node) {
        // We only add to data center level since the root must already has everything
        for (int j = 0; j < NetworkTopology.DEFAULT_HOST_LEVEL - 1; j++) {
            ...
            LOG.info("==elf== adding prefetched replica on node " + tip.getTIPId() + " on node " + node.getName());
            node = node.getParent();
        }
    }
}
