...
class TaskInProgress {
    ...
    public boolean isRunning() {
        //return (!isDCLocal() && getPrefetchStatus() == PrefetchStatus.RUNNING) ||
        //        (!activeTasks.isEmpty());
    ...
    //
    private boolean isCompleteTimePredicted = false;

    public boolean isCompleteTimePredicted() {
        return isCompleteTimePredicted;
    }

    public void markAsPredicted() {
        isCompleteTimePredicted = true;
    }

    private boolean hasReplicaInCluster = true;

    /**
     * To mark that this task has no replica on any task tracker in the running mapreduce cluster
     * */
    public void markAsRemoteDCTask(){
        this.hasReplicaInCluster = false;
        this.prefetchStatus = PrefetchStatus.NONE_RUNNING;
    }

    public boolean isDCLocal(){
        return hasReplicaInCluster;
    }

    public static enum PrefetchStatus {NONE_RUNNING, RUNNING, FINISHED}

    private PrefetchStatus prefetchStatus = PrefetchStatus.NONE_RUNNING;
    public PrefetchStatus getPrefetchStatus(){
        return prefetchStatus;
    }
    public void markPrefetchStatus(PrefetchStatus status){
        prefetchStatus = status;
    }

    private PrefetchTask futurePrefetch = null;

    /**
     * Add a prefetch task for future use
     * */
    public void addPrefetchTask(TaskSplitMetaInfo split) {
        this.futurePrefetch = new PrefetchTask(split, this);
    }

    public PrefetchTask getFuturePrefetch() {
        return futurePrefetch;
    }

    public boolean isPrefetchingRunning(){
        return isMapTask() && !isDCLocal() && prefetchStatus == PrefetchStatus.RUNNING;
    }
}
